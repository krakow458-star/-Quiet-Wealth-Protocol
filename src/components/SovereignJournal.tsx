...
export const SovereignJournal: React.FC<{ day: number; name: string; task: string }> = ({ day, name, task }) => {
  ...
  return (
    <div className="glass-panel rounded-2xl overflow-hidden border-gold/20 shadow-2xl">
      <div className="bg-gold/10 p-4 border-b border-gold/20 flex justify-between items-center">
        ...
        <div className="flex items-center gap-2">
           <div className={`w-2 h-2 rounded-full ${currentEntry ? 'bg-green-500 shadow-[0_0_8px_#22c55e]' : 'bg-red-500 animate-pulse'}`} />
      ...
      <div className="p-6 space-y-6">
        <div className="space-y-4">
           <div className="p-4 bg-black/40 border border-white/5 rounded-xl">
              <div className="text-[8px] text-gold font-bold uppercase tracking-widest mb-2 flex items-center gap-2">
           ...
           <div className="relative">
              <textarea 
                ...
                className="w-full h-32 bg-white/5 border border-white/10 rounded-xl p-4 text-[11px] text-white font-mono placeholder:text-muted/30 outline-none focus:border-gold/30 transition-all resize-none"
              />
           ...
           <button 
             ...
             className={`w-full py-4 rounded-xl text-[10px] font-black uppercase tracking-[0.3em] flex items-center justify-center gap-3 transition-all ${
               isSaved ? 'bg-green-500/20 text-green-400 border border-green-500/30' : 'bg-gold text-black hover:scale-[1.02] active:scale-95'
             }`}
           >
          ...
          <div className="pt-4 border-t border-white/5">
             ...
             <div className="flex gap-2 overflow-x-auto pb-2 scrollbar-none">
                {history.sort((a, b) => b.day - a.day).slice(0, 5).map(entry => (
                   <div key={entry.day} className="flex-shrink-0 p-2 bg-white/2 border border-white/5 rounded-lg flex items-center gap-2 min-w-[60px] justify-center opacity-60 hover:opacity-100 transition-opacity">
                      <span className="text-[8px] text-gold font-bold">D{entry.day}</span>
      ...
      <div className="bg-gold/5 p-3 px-6 text-[8px] text-gold/40 uppercase tracking-[0.3em] italic">
